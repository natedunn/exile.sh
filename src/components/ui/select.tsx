    ...
    <SelectPrimitive.Portal>
      ...
      >
        <SelectPrimitive.Popup
          ...
          className={cn(
            "relative isolate z-50 max-h-(--available-height) w-(--anchor-width) min-w-36 origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded border border-rule-strong bg-surface font-mono text-xs text-ink shadow-menu duration-120",
            className
          )}
          {...props}
        >
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.GroupLabel
      ...
      className={cn("px-1.5 py-1 text-xs text-ink-muted", className)}
      {...props}
    />
  )
}
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-1.5 rounded-none py-1.5 pr-8 pl-2 text-xs outline-hidden select-none focus:bg-hover focus:text-ink data-disabled:pointer-events-none data-disabled:opacity-50 data-highlighted:bg-hover data-highlighted:text-ink [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...props}
    >
