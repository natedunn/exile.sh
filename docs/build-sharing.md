...
- `PopoverContent variant="inspection"` in `src/components/ui/popover.tsx` owns the shared frame, title, dither, and scrolling defaults. `InspectionTooltipContent` adds hover interactivity and pin controls. Customize `--inspection-width`, `--inspection-max-height`, `--inspection-padding`, `--inspection-accent`, and `--inspection-border` through token-based Tailwind classes instead of copying the shell.
...
Use the shared shell for future rich tooltips; put content-specific Tailwind utilities in the content component or shared class lists such as `src/components/equipment-classes.ts`. Add missing design tokens to `tokens.css`; only reusable texture/renderer effects belong in `src/styles.css` as `@utility` definitions. Do not add page/content stylesheets or style primitives externally through `[data-slot]` selectors. Check equipment, skills, passive-tree, and tooltip-pins browser suites after changing shared behavior.
