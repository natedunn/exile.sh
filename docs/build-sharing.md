...
The existing Convex provider and kitcn procedures are reused. Set the normal development environment variables from `.env.example`, run `bun run convex:dev` to sync the schema/functions, and run `bun run dev`. Set `VITE_SITE_URL` to the public origin for canonical and Open Graph URLs. The development backend has been synced during implementation. Preview builds use the development deployment without deploying backend changes; the existing main-branch build pipeline deploys the Convex backend before publishing the frontend. Run `bun run codegen` if generated files need refreshing. Do not point a preview at production.
